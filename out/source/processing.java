...
class Board {
  ...
  public void loadBoard(String levelPath) {
    init();
  ...
  public PVector getDirectionFromKey(int keyCode) {
    if(keyCode == LEFT) {
  ...
  public boolean isBox(PVector pos) {
  ...
  public boolean isFree(PVector pos) {
    return isWalkable(grid[(int)pos.y][(int)pos.x]) && !isBox(pos);
  }

  public Cell cell(PVector pos) {
    return grid[(int) pos.y][(int) pos.x];
  }

  public boolean boxCanMove(PVector box, PVector dir) {
    PVector next = PVector.add(box, dir);
    return isFree(next);
  }

  public PVector getBoxAt(PVector pos) {
    for(int i = 0; i < boxes.size(); i++){
      if(boxes.get(i).equals(pos)) {
        return boxes.get(i);
      }
  ...
  public void moveBox(PVector box, PVector dir) {
  ...
  public Direction toEnum(PVector direction) {
  ...
  public void movePlayer(PVector direction) {
  ...
  public void drawBox(PVector box) {
    // fill(0, 128, 0);
